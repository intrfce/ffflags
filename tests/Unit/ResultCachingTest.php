...
beforeEach(function () {
    ...
    BypassStorageFeature::$resolveCount = 0;
});
...
it('stores result in database after first evaluation', function () {
    ...
    expect($record->feature_slug)->toBe('scoped-feature');
...
it('caches scopeless results with null scope fields', function () {
    ...
    $record = FeatureFlagResult::first();
    expect($record->feature_slug)->toBe('always-active');
...
it('uses in-memory cache with BypassStorage attribute within same manager', function () {
    BypassStorageFeature::for(null)->isActive();
    BypassStorageFeature::for(null)->isActive();
    BypassStorageFeature::for(null)->isActive();

    expect(BypassStorageFeature::$resolveCount)->toBe(1);
});

it('re-resolves with BypassStorage attribute on fresh manager', function () {
    BypassStorageFeature::for(null)->isActive();
    expect(BypassStorageFeature::$resolveCount)->toBe(1);
    ...
    // Without database cache, it must resolve again.
    BypassStorageFeature::for(null)->isActive();
    expect(BypassStorageFeature::$resolveCount)->toBe(2);
});

it('does not store result in database when BypassStorage attribute is present', function () {
    BypassStorageFeature::for(null)->isActive();
